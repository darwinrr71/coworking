...
export default function RoomCreatePage() {
  ...
  const {
    ...
    defaultValues: { type: "KontorCoworking" },
  });
  ...
  if (me?.user?.role !== "Admin") {
    return (
      <div className="mx-auto w-full max-w-xl px-4 py-16 sm:px-6">
        <p className="text-sm text-red-500">Du saknar behörighet för att skapa rum.</p>
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto w-full max-w-xl px-4 py-16 sm:px-6">
      <div className="glass-panel rounded-[32px] p-8">
        <h1 className="text-3xl font-semibold text-(--color-deep)">Skapa nytt rum</h1>
        <form
        ...
        >
          ...
          <Button type="submit" disabled={mutation.isPending} className="w-full">
            {mutation.isPending ? "Skapar..." : "Skapa rum"}
          </Button>
        </form>
        {mutation.isError && (
        ...
        {mutation.isSuccess && (
          <p className="mt-4 text-sm text-(--color-forest)">Rummet är skapat.</p>
        )}
      </div>
    </div>
  );
